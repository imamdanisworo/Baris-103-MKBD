...
def html_table(df, numeric_cols, colgroup=""):
    ...
    for _, row in df.reset_index(drop=True).iterrows():
        ...
        for col in df.columns:
            ...
            style = base + " font-weight:bold; background-color:#f2f2f2;" if row['custname'] == 'TOTAL' else base
            r += f"<td style='{style} padding:4px 10px'>{row[col]}</td>"
        r += "</tr>"
        rows.append(r)
    headers = "".join(f"<th style='text-align:left; padding:4px 10px; white-space: nowrap;'>{c}</th>" for c in df.columns)
...
if 'final' in st.session_state and 'total_row' in st.session_state:
    ...
    final_df = st.session_state['final'].rename(columns=colnames)
    total_df = st.session_state['total_row'].rename(columns=colnames)

    st.subheader("📋 All Clients Balance Comparison")
    st.markdown("##### Detail Data")
    st.dataframe(add_separator(final_df, list(colnames.values())), use_container_width=True)
    st.markdown("##### TOTAL")
    st.markdown(html_table(add_separator(total_df, list(colnames.values())), list(colnames.values())), unsafe_allow_html=True)
