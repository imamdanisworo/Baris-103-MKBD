...
if file_y and file_c and st.button("🚦 Generate Comparison Table"):
    with st.spinner("Processing..."):
        ...
        totals = {
          ...
          'bal_y': final['bal_y'].sum(),'bal_c': final['bal_c'].sum(),
          'change': final['change'].sum(),'int_rate':None,'int_rate_daily':None
        }
...
if 'final' in st.session_state:
    ...
    tr = st.session_state['total_row'].rename(columns=colnames)
    st.subheader("📋 All Clients Balance Comparison")
    styled_main = add_separator(df_main, list(colnames.values()))
    styled_tot = add_separator(tr, list(colnames.values()))
    cg = get_colgroup_by_width(pd.concat([df_main, tr]), list(colnames.values()))
    st.markdown(html_table(styled_main, list(colnames.values()), cg), unsafe_allow_html=True)
    st.markdown(html_table(styled_tot, list(colnames.values()), cg), unsafe_allow_html=True)

    csv = pd.concat([df_main, tr]).to_csv(index=False).encode('utf-8')
    ...
    def sum_table(d):
        ...
        t = pd.DataFrame([{
            'Group':'Total','Fee Type':'','bal_y':s.bal_y.sum(),
            'bal_c':s.bal_c.sum(),'change':s.change.sum()
        }])
    ...
    with tab_analysis:
        for title,tbl in [
            ...
            st.markdown(f"#### {title}")
            disp = tbl.rename(columns=colnames)
            styled = add_separator(disp, list(colnames.values()))
            cg2 = get_colgroup_by_width(styled, list(colnames.values()))
            st.markdown(html_table(styled, list(colnames.values()), cg2), unsafe_allow_html=True)

    # 📌 Now the new summary tables you requested (below existing analysis):
    # Positive Changes Summary
    pos = df[df['change'] > 0]
    buckets_pos = [
      ('< 500 Mio', pos[pos['change'] < 500_000_000]),
      ('500M - 1B', pos[(pos['change'] >= 500_000_000) & (pos['change'] <= 1_000_000_000)]),
      ('> 1B', pos[pos['change'] > 1_000_000_000])
    ]
    pos_summary = pd.DataFrame([
      {'Range': r,'Client Count':len(g),'Total Changes':g['change'].sum()}
      for r,g in buckets_pos
    ])

    # Negative Changes Summary (sorted > -500, -1B.., <-1B)
    neg = df[df['change'] < 0]
    buckets_neg = [
      ('> -500 Mio', neg[neg['change'] > -500_000_000]),
      ('-1B to -500M', neg[(neg['change'] <= -500_000_000) & (neg['change'] >= -1_000_000_000)]),
      ('< -1B', neg[neg['change'] < -1_000_000_000])
    ]
    neg_summary = pd.DataFrame([
      {'Range': r,'Client Count':len(g),'Total Changes':g['change'].sum()}
      for r,g in buckets_neg
    ])

    st.markdown("### 📊 Additional: Change‑Amount Distribution")
    st.markdown("#### ➕ Positive Changes by Bucket")
    st.markdown(
      html_table(
        add_separator(pos_summary, ['Total Changes']),
        ['Total Changes'],
        get_colgroup_by_width(pos_summary, ['Total Changes'])
      ),
      unsafe_allow_html=True
    )

    st.markdown("#### ➖ Negative Changes by Bucket")
    st.markdown(
      html_table(
        add_separator(neg_summary, ['Total Changes']),
        ['Total Changes'],
        get_colgroup_by_width(neg_summary, ['Total Changes'])
      ),
      unsafe_allow_html=True
    )
