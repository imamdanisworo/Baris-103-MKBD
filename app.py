...
if 'final' in st.session_state:
    ...
    # Render just the TOTAL row with matching column widths
    combined = pd.concat([df_disp, total_disp], ignore_index=True)
    cg = get_colgroup_by_width(combined, list(cols.values()))
    st.markdown(html_table(add_separator(total_disp, list(cols.values())), list(cols.values()), cg),
    ...
    def sum_table(d):
        ...
        t = pd.DataFrame([{'Group':'Total','Fee Type':'',
    ...
    def total_only(d):
        ...
        g = pd.DataFrame([{'Fee Type':'Grand Total',
